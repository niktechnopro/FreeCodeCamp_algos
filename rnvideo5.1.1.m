...
- (BOOL)setLicenseResultError:(NSString *)error {
  if (_loadingRequest != nil) {
    ...
    [self finishLoadingWithError:licenseError  httpStatusCode:nil];
  }
  return NO;
}

- (BOOL)finishLoadingWithError:(NSError *)error httpStatusCode:(NSNumber *)httpStatusCode {
  if (_loadingRequest && error != nil) {
    ...
    if (self.onVideoError) {
      self.onVideoError(@{@"error": @{@"code": [NSNumber numberWithInteger: error.code],
                          ...
                          @"target": self.reactTag,
                          @"httpStatusCode": httpStatusCode == nil ? [NSNull null] : httpStatusCode
                        });
    }
  }
  return NO;
  ...
  if (self->_drm != nil) {
    ...
    if ([drmType isEqualToString:@"fairplay"]) {
      ...
      if (certificateStringUrl != nil) {
          ...
          if (certificateData != nil) {
            ...
            if (dataRequest != nil) {
              ...
              NSString *licenseServer = (NSString *)[self->_drm objectForKey:@"licenseServer"];
                [self checkTicketStatus:[NSURL URLWithString:licenseServer] completion:^(int httpCode) {
                    if (httpCode == 412) {
                        NSError *error412 = [NSError errorWithDomain: @"RCTVideo"
                        ...
                        [self finishLoadingWithError:error412 httpStatusCode:@412];
                    }
                }];
              if (spcError != nil) {
                  [self finishLoadingWithError:spcError httpStatusCode:nil];
                ...
                if(self.onGetLicense) {
                  ...
                  NSURLSessionDataTask *postDataTask = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                    ...
                    if (error != nil) {
                      ...
                      [self finishLoadingWithError:error httpStatusCode:nil];
                      self->_requestingCertificateErrored = YES;
                    } else {
                      if([httpResponse statusCode] != 200){
                        ...
                        if(self.onGetLicense) {
                          ...
                          if ([self->_drm objectForKey:@"renewal"]) {
                              NSString *drmRenewal = (NSString *)[self->_drm valueForKey:@"renewal"];
                              AVAssetResourceLoadingContentInformationRequest *request = [loadingRequest contentInformationRequest];
                              double renewalInterval = drmRenewal == nil ? 20 : [drmRenewal doubleValue];
                              request.renewalDate = [NSDate dateWithTimeIntervalSinceNow:renewalInterval];
                          }
        ...
        return [self finishLoadingWithError:licenseError httpStatusCode:nil];
      }
    } else {
      NSError *licenseError = [NSError errorWithDomain: @"RCTVideo"
...
- (void)checkTicketStatus:(NSURL*)url completion:(void (^)(int httpCode))completion {
